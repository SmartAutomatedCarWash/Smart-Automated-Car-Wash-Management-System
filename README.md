...
graph TD
    ...
    subgraph "Frontend — Next.js 14 App Router"
        ...
        MP["Manager Portal — /manager/*"]:::client
        AP["Admin Portal — /admin/*"]:::client
    end

    subgraph "Backend — Spring Boot 3.3.5"
        API["REST API Layer<br/>Controllers · Services · DTOs · MapStruct Mappers · Assemblers"]:::server
        JOBS["Scheduled Jobs<br/>No-show Detection · Slot Cleanup · Booking Reminder"]:::server
    end

    subgraph "Data Layer"
        DB[("PostgreSQL<br/>Flyway · Schema V1 Baseline + Demo Seed")]:::db
    end

    CP & SP & MP & AP -->|REST| API
...
| **Backend** | Spring Boot 3.3.5 · Java 21 · Spring Security · Spring Data JPA · Flyway · JWT (JJWT) · Lombok · MapStruct |
| **Database** | PostgreSQL · Flyway migrations (`V1__init_db_squashed.sql`) · Demo seed (`V100__seed_demo_data.sql`) |
...
│   └── src/
│       ├── app/                       # App Router (role-based route groups + auth)
│       ├── entities/                  # Business entity types & interfaces
│       ├── features/                  # Feature modules (auth, bookings, loyalty, discounts, ...)
...
│       ├── assembler/                 # Aggregate response assemblers
...
│       └── shared/                    # Security, config, error codes, utilities
│
└── docs/                              # Technical documentation
    ├── master/                        # PROJECT.md · BUSINESS_RULES.md
    └── context/                       # Backend/frontend/schema context files
```
...
- Node.js ≥ 18.18
- Java JDK 21
- Maven wrapper from `autowash-backend/mvnw.cmd`
- PostgreSQL
