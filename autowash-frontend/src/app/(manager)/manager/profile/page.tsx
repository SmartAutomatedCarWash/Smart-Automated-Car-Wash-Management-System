...
} from "@/features/profile/hooks/use-manager-profile";
...
export default function ManagerProfilePage() {
  ...
  const submitMessage = updateProfileMutation.isError
    ? updateProfileMutation.error.errors?.map((error) => error.message).join(" ") ||
      ...
      <div className="w-full max-w-sm">
        {view === "profile" && (
          <div className="rounded-[24px] bg-white p-6 shadow-[0_8px_30px_rgb(0,0,0,0.06)] ring-1 ring-slate-100 sm:p-8">
            ...
            <div className="mt-8">
              <h2 className="mb-4 text-sm font-semibold text-slate-800">Personal information</h2>
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <ProfileField
                  ...
                  inputMode="email"
                  disabled
                  hint="Email is managed by Admin and cannot be changed"
                  error={null}
                />
                <ProfileField
                  label="Phone number"
          ...
          <div className="rounded-[24px] bg-white p-6 shadow-[0_8px_30px_rgb(0,0,0,0.06)] ring-1 ring-slate-100 sm:p-8">
            ...
            <form onSubmit={handlePasswordSubmit} className="space-y-5">
              <PasswordField
                ...
                onToggle={() => setShowPasswords((passwords) => ({ ...passwords, current: !passwords.current }))}
              ...
              <PasswordField
                ...
                onToggle={() => setShowPasswords((passwords) => ({ ...passwords, newPass: !passwords.newPass }))}
              ...
              <PasswordField
                ...
                onToggle={() => setShowPasswords((passwords) => ({ ...passwords, confirm: !passwords.confirm }))}
              ...
              <div className="pt-2">
                <Button
                  ...
                  className="h-12 w-full rounded-full bg-[#0D3B3A] text-base font-semibold text-white hover:bg-[#155452]"
                >
    ...
    <div>
      ...
      >
        ...
        <button
          type="button"
          onClick={onToggle}
          className="ml-2 flex-shrink-0 text-slate-300 transition-colors hover:text-slate-500"
          tabIndex={-1}
...
function validateForm(form: ProfileFormState) {
  return {
    ...
    phone: !phonePattern.test(form.phone.trim())
      ? "Phone number must match the 0XXXXXXXXX format."
      : null,
  };
}
...
function validatePasswordForm(form: PasswordFormState) {
  ...
  return {
    currentPassword: form.currentPassword.length === 0 ? "Enter your current password." : null,
    newPassword:
