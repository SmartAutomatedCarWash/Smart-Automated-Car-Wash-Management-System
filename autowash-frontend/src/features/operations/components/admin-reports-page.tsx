...
export function AdminReportsPage() {
  ...
  return (
    <WorkspacePage compact className="max-w-none space-y-3 bg-[#fbfdff] px-4 pb-5 pt-3 lg:px-5">
      ...
      {query.isError ? (
        <WorkspaceEmptyState title={t("Không thể tải báo cáo hệ thống", "Unable to load system reports")} description={getErrorMessage(query.error as unknown as ApiErrorResponse)} />
      ) : (
        <>
          <section className="grid gap-3 md:grid-cols-2 xl:grid-cols-5">
            <MetricCard icon={TrendingUp} label={t("Doanh thu toàn hệ thống", "System revenue")} value={formatCurrency(revenue, locale)} detail={t("Chỉ tính booking hoàn thành và đã thanh toán", "Completed sessions only")} tone="emerald" />
            <MetricCard icon={CalendarDays} label={t("Tổng booking hoàn thành", "Total completed bookings")} value={String(completedBookings)} detail={t(`trên ${totalBookings} booking trong kỳ`, `of ${totalBookings} bookings`)} tone="blue" />
            <MetricCard icon={BarChart3} label={t("Tỷ lệ hoàn thành hệ thống", "System completion rate")} value={`${completionRate}%`} detail={comparePrevious ? t("↓ 5% so với kỳ trước", "↓ 5% vs previous") : t("Theo bộ lọc hiện tại", "Current filter")} tone="amber" />
            <MetricCard icon={Target} label={t("Ticket trung bình toàn hệ thống", "System average ticket")} value={formatCurrency(averageTicket, locale)} detail={t("Doanh thu / booking hoàn thành", "Revenue / completed booking")} tone="slate" />
            <MetricCard icon={Star} label={t("Rating trung bình hệ thống", "System average rating")} value={formatNullableRating(averageRating)} detail={`${reviewCount} ${t("đánh giá", "reviews")}`} tone="yellow" rating={averageRating ?? 0} />
          </section>

          <Card className="rounded-2xl border-amber-100 bg-white p-3 shadow-sm">
            <div className="mb-2 flex items-center gap-2">
              <AlertTriangle className="h-4 w-4 text-amber-500" />
              <h2 className="text-sm font-black text-slate-950">{t("Cảnh báo & Rủi ro hệ thống", "System Attention & Risks")}</h2>
            </div>
            <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
              ...
              <AttentionCard icon={CalendarDays} title={t("Booking tồn hệ thống", "System unfinished bookings")} value={`${unfinishedBookings} ${t("booking chưa hoàn thành", "unfinished bookings")}`} action={t("Kiểm tra ngay", "Review now")} tone="amber" />
...
function FunnelPanel({ rows, total, language }: { rows: FunnelRow[]; total: number; language: Language }) {
  return (
    <Card className="rounded-2xl border-slate-200 bg-white p-4 shadow-sm">
      <h2 className="text-sm font-black text-slate-950">{translate(language, "Funnel vận hành toàn hệ thống", "System operations funnel")}</h2>
...
function RevenuePanel({ rows, language, locale }: { rows: TrendRow[]; language: Language; locale: string }) {
  ...
  return (
    <Card className="rounded-2xl border-slate-200 bg-white p-4 shadow-sm">
      <h2 className="text-sm font-black text-slate-950">{translate(language, "Doanh thu & booking hệ thống", "System revenue & bookings")}</h2>
...
function ServiceQualityPanel({ rows, averageRating, reviewCount, feedbackToReview, language }: { rows: ServiceRow[]; averageRating: number | null; reviewCount: number; feedbackToReview: number; language: Language }) {
  return (
    <Card className="rounded-2xl border-slate-200 bg-white p-4 shadow-sm">
      <h2 className="text-sm font-black text-slate-950">{translate(language, "Chất lượng dịch vụ hệ thống", "System service quality")}</h2>
...
function buildFunnelRows(sessions: OperationsQueueSession[], language: Language): FunnelRow[] {
  ...
  return [
    { key: "total", label: translate(language, "Booking toàn hệ thống", "System bookings"), count: sessions.length, rate: 100, barPercent: sessions.length > 0 ? 100 : 0, helper: "", color: "bg-[#1687ee]" },
